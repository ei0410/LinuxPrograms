...
static long context_length(struct HTTPRequest *req)
{
	char *val;
	long len;
	...
	if (strcmp(req->method, "HEAD") != 0) {
		...
		for (;;) {
			...
			if (fwrite(buf, n, 1, out) < n)
				log_exit("failed to write %s: %s", info->path, strerror(errno));
		}
		close(fd);
	}
	fflush(out);
